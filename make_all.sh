#!/bin/sh
OPTS='-j 2 -e EXECUTE_ON_HIGHEST=no -e QWERTY_BASE=no'
...
# qmk compile -kb boardsource/technik_o -km wheredoesyourmindgo -j 2 -e EXECUTE_ON_HIGHEST=no -e QWERTY_BASE=no -e RGB_MATRIX_ENABLE=yes -e KEY_LOCK_ENABLE=no
...
# qmk compile -kb planck/rev6 -km wheredoesyourmindgo -j 2 -e EXECUTE_ON_HIGHEST=no -e QWERTY_BASE=no -e ENCODER_ENABLE=yes -e LOWER_LEFT_ENCODER=yes
# qmk compile -kb planck/rev6 -km wheredoesyourmindgo -j 2 -e EXECUTE_ON_HIGHEST=no -e QWERTY_BASE=yes
